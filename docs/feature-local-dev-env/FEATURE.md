...
- [x] `p2` - **ID**: `cpt-katapult-flow-local-dev-env-provision`
...
**Steps**:
1. [x] - `p2` - Developer runs `make local-up` (default: single-cluster) or `make local-up MODE=multi` - `inst-run-local-up`
2. [x] - `p2` - **IF** existing local environment detected (Kind cluster exists with katapult prefix), prompt developer to tear down first or abort - `inst-check-existing`
3. [x] - `p2` - Validate prerequisites: Docker running, Kind installed, kubectl installed, Go installed - `inst-validate-prereqs`
4. [x] - `p2` - **IF** any prerequisite missing **RETURN** error listing missing tools with install instructions - `inst-prereq-fail`
5. [x] - `p2` - Provision Kind cluster(s) using algorithm `cpt-katapult-algo-local-dev-env-provision-kind` - `inst-provision-kind`
6. [x] - `p2` - Deploy supporting services using algorithm `cpt-katapult-algo-local-dev-env-deploy-services` - `inst-deploy-services`
7. [x] - `p2` - Deploy Katapult stack using algorithm `cpt-katapult-algo-local-dev-env-deploy-stack` - `inst-deploy-stack`
8. [x] - `p2` - Provision seed data using flow `cpt-katapult-flow-local-dev-env-seed` - `inst-seed-data`
9. [x] - `p2` - Wait for all pods to be Ready and agents to register with control plane - `inst-wait-ready`
10. [x] - `p2` - Print environment summary: cluster name(s), API endpoint, Web UI URL, MinIO console URL (if multi), number of registered agents, number of PVCs available - `inst-print-summary`
11. [x] - `p2` - **RETURN** environment ready - `inst-return-ready`

### Tear Down Local Environment

- [x] `p2` - **ID**: `cpt-katapult-flow-local-dev-env-teardown`
...
**Steps**:
1. [x] - `p2` - Developer runs `make local-down` - `inst-run-local-down`
2. [x] - `p2` - **IF** no local environment detected, print "No local environment found" and **RETURN** - `inst-check-nothing`
3. [x] - `p2` - Clean up resources using algorithm `cpt-katapult-algo-local-dev-env-cleanup` - `inst-cleanup`
...
6. [x] - `p2` - Print teardown summary: resources removed - `inst-print-teardown`
7. [x] - `p2` - **RETURN** environment torn down - `inst-return-torn-down`

### Rebuild Component

- [x] `p2` - **ID**: `cpt-katapult-flow-local-dev-env-rebuild`
...
**Steps**:
1. [x] - `p2` - Developer runs `make local-rebuild COMPONENT=<name>` - `inst-run-rebuild`
2. [x] - `p2` - **IF** no local environment detected **RETURN** error "No local environment running. Run make local-up first." - `inst-check-env-exists`
3. [x] - `p2` - **IF** COMPONENT not in [controlplane, agent, webui] **RETURN** error listing valid component names - `inst-validate-component`
4. [x] - `p2` - Build Go binary for the specified component - `inst-build-binary`
5. [x] - `p2` - Build container image with the updated binary - `inst-build-image`
6. [x] - `p2` - Load container image into Kind cluster(s) - `inst-load-image`
7. [x] - `p2` - Restart the Kubernetes Deployment or DaemonSet for the component to pick up the new image - `inst-restart-deployment`
8. [x] - `p2` - Wait for rollout to complete (pods Ready) - `inst-wait-rollout`
9. [x] - `p2` - **RETURN** component rebuilt and redeployed - `inst-return-rebuilt`

### Seed Data Provisioning

- [x] `p2` - **ID**: `cpt-katapult-flow-local-dev-env-seed`
...
**Steps**:
1. [x] - `p2` - Create PVCs with sample data on each worker node (varying sizes: small 100MB, medium 1GB, large 5GB) - `inst-create-pvcs`
2. [x] - `p2` - Populate PVCs with realistic sample data using a busybox job (random files with directory structures) - `inst-populate-data`
3. [x] - `p2` - Wait for agents to register with the control plane (poll agent list API) - `inst-wait-agents`
4. [x] - `p2` - **IF** agents not registered within 60 seconds **RETURN** error listing unregistered nodes - `inst-agent-timeout`
5. [x] - `p2` - Trigger a sample intra-cluster transfer and wait for completion to populate transfer history - `inst-sample-transfer`
6. [ ] - `p2` - **IF** MODE=multi, trigger a sample cross-cluster S3-staged transfer - `inst-sample-cross-transfer`
7. [x] - `p2` - **RETURN** seed data provisioned - `inst-return-seeded`

## 3. Processes / Business Logic (CDSL)

### Provision Kind Cluster

- [x] `p2` - **ID**: `cpt-katapult-algo-local-dev-env-provision-kind`
...
**Steps**:
1. [x] - `p2` - Generate Kind cluster configuration: 1 control-plane node + WORKERS worker nodes, with extraPortMappings for API server and Web UI access - `inst-gen-kind-config`
2. [x] - `p2` - **IF** MODE=single, create one Kind cluster named `katapult-dev` - `inst-create-single`
3. [ ] - `p2` - **IF** MODE=multi, create two Kind clusters named `katapult-dev-src` and `katapult-dev-dst` - `inst-create-multi`
4. [x] - `p2` - Configure kubeconfig contexts for created cluster(s) - `inst-configure-kubeconfig`
...
6. [x] - `p2` - **RETURN** cluster(s) ready with kubeconfig configured - `inst-return-clusters`

### Deploy Supporting Services

- [x] `p2` - **ID**: `cpt-katapult-algo-local-dev-env-deploy-services`

**Input**: Cluster mode, port configuration

**Output**: PostgreSQL and MinIO running and accessible

**Note**: Implementation uses in-cluster K8s deployments (reusing `deploy/test/` manifests via kustomize overlay) instead of docker-compose. This is simpler and avoids docker-compose-to-Kind networking complexity.

**Steps**:
1. [x] - `p2` - Generate docker-compose.yaml with PostgreSQL service (port POSTGRES_PORT) - `inst-gen-compose`
2. [ ] - `p2` - **IF** MODE=multi, add MinIO service to docker-compose.yaml (ports MINIO_PORT, MINIO_CONSOLE_PORT) - `inst-add-minio`
3. [x] - `p2` - **IF** MODE=single, add MinIO service for optional local S3 testing - `inst-add-minio-single`
4. [x] - `p2` - Run `docker-compose up -d` to start services - `inst-start-services`
5. [x] - `p2` - Wait for PostgreSQL to accept connections (poll with pg_isready or TCP check) - `inst-wait-postgres`
6. [x] - `p2` - Run database migrations (create schema, tables) against PostgreSQL - `inst-run-migrations`
7. [ ] - `p2` - **IF** MinIO started, create the S3 bucket for transfer staging using MinIO client - `inst-create-bucket`
8. [x] - `p2` - **RETURN** supporting services ready - `inst-return-services`

### Deploy Katapult Stack

- [x] `p2` - **ID**: `cpt-katapult-algo-local-dev-env-deploy-stack`
...
**Steps**:
1. [x] - `p2` - Build Go binaries for control plane and agent (`go build`) - `inst-build-binaries`
2. [x] - `p2` - Build container images for control plane, agent, and Web UI - `inst-build-images`
3. [x] - `p2` - Load container images into Kind cluster(s) using `kind load docker-image` - `inst-load-images`
4. [x] - `p2` - Apply CRD manifests (VolumeTransfer CRD) to cluster(s) - `inst-apply-crds`
5. [x] - `p2` - Apply control plane Deployment manifest (references PostgreSQL and MinIO connection details via environment variables) - `inst-deploy-controlplane`
6. [x] - `p2` - Apply agent DaemonSet manifest to worker nodes - `inst-deploy-agents`
7. [x] - `p2` - Apply Web UI Deployment manifest with Service exposing on WEBUI_PORT - `inst-deploy-webui`
8. [ ] - `p2` - **IF** MODE=multi, deploy agents and control plane connectivity to both clusters - `inst-multi-deploy`
9. [x] - `p2` - Wait for all Deployments and DaemonSets to reach Ready state - `inst-wait-deployments`
10. [x] - `p2` - **RETURN** Katapult stack deployed - `inst-return-stack`

### Cleanup Resources

- [x] `p2` - **ID**: `cpt-katapult-algo-local-dev-env-cleanup`

**Input**: None (discovers existing resources by naming convention)

**Output**: All local environment resources removed

**Note**: Implementation uses `kind delete cluster` which handles all cluster resources. No docker-compose teardown needed since services run in-cluster.

**Steps**:
1. [x] - `p2` - Delete Kind cluster(s) matching katapult prefix (`kind delete cluster --name katapult-dev*`) - `inst-delete-clusters`
2. [x] - `p2` - Stop and remove docker-compose services (`docker-compose down -v`) - `inst-stop-services`
...
6. [x] - `p2` - **RETURN** all resources cleaned up - `inst-return-cleaned`
...
- [x] `p2` - **ID**: `cpt-katapult-dod-local-dev-env-provision`
...
- [x] `p2` - **ID**: `cpt-katapult-dod-local-dev-env-single-cluster`
...
- [x] `p2` - **ID**: `cpt-katapult-dod-local-dev-env-seed-data`
...
- [x] `p2` - **ID**: `cpt-katapult-dod-local-dev-env-fast-rebuild`
...
- [x] `p2` - **ID**: `cpt-katapult-dod-local-dev-env-teardown`
...
- [x] `make local-up` provisions a single-cluster environment with control plane, agents, and Web UI running
- [x] Single-cluster environment is transfer-ready (agents registered, seed data loaded, Web UI accessible) within 3 minutes
- [ ] `make local-up MODE=multi` provisions two Kind clusters with MinIO for cross-cluster testing
- [x] An intra-cluster streaming transfer completes in the single-cluster environment
- [ ] A cross-cluster S3-staged transfer completes in the multi-cluster environment
- [x] Seed data includes PVCs with sample data on worker nodes, registered agents, and transfer history
- [x] `make local-rebuild COMPONENT=controlplane` rebuilds and redeploys only the control plane without reprovisioning clusters
- [x] `make local-rebuild COMPONENT=agent` rebuilds and redeploys only the agent DaemonSet
- [x] `make local-down` removes all Kind clusters, docker containers, networks, and volumes
- [ ] No orphaned docker resources (containers, networks, volumes) remain after teardown
- [x] Web UI is accessible at localhost:WEBUI_PORT after provisioning
