...
- [ ] `p1` - **ID**: `cpt-katapult-flow-api-cli-create-transfer-api`
...
**Steps**:
1. [ ] - `p1` - Operator sends POST /api/v1alpha1/transfers with source, destination, optional strategy override, allowOverwrite, and retry config - `inst-submit-request`
2. [ ] - `p1` - API Server authenticates request via auth middleware (local credentials or OIDC token) - `inst-authenticate`
3. [ ] - `p1` - API Server authorizes request via RBAC middleware (require operator role) - `inst-authorize`
4. [ ] - `p1` - API Server validates request body using `cpt-katapult-algo-api-cli-validate-transfer-request` - `inst-validate-input`
5. [ ] - `p1` - **IF** validation fails **RETURN** 400 with field-level error details - `inst-return-validation-error`
6. [ ] - `p1` - API Server delegates to Transfer Orchestrator via `cpt-katapult-component-transfer-orchestrator` CreateTransfer - `inst-delegate-create`
7. [ ] - `p1` - **RETURN** 201 Created with transfer ID, status=Pending, and resource URL - `inst-return-created`

### Create Transfer via CLI

- [ ] `p1` - **ID**: `cpt-katapult-flow-api-cli-create-transfer-cli`
...
**Steps**:
1. [ ] - `p1` - Operator runs `katapult transfer create --source-cluster <cluster> --source-pvc <ns/name> --dest-cluster <cluster> --dest-pvc <ns/name>` with optional flags --strategy, --allow-overwrite, --output - `inst-cli-invoke`
2. [ ] - `p1` - CLI resolves API server address from kubeconfig context or --server flag - `inst-resolve-server`
3. [ ] - `p1` - CLI constructs POST /api/v1alpha1/transfers request using `cpt-katapult-algo-api-cli-resolve-cli-command` - `inst-construct-request`
4. [ ] - `p1` - CLI sends authenticated request to API Server (bearer token from kubeconfig or --token flag) - `inst-send-request`
5. [ ] - `p1` - **IF** API returns error **RETURN** formatted error message with remediation hint - `inst-handle-error`
6. [ ] - `p1` - CLI formats response per --output flag (table default, json, yaml) and displays transfer ID, status, source, destination - `inst-format-output`

### List Transfers via API

- [ ] `p1` - **ID**: `cpt-katapult-flow-api-cli-list-transfers`
...
**Steps**:
1. [ ] - `p1` - Operator sends GET /api/v1alpha1/transfers with optional query parameters (status, cluster, limit, offset) or runs `katapult transfer list` - `inst-query-transfers`
2. [ ] - `p1` - API Server authenticates and authorizes request (viewer or operator role) - `inst-auth-list`
3. [ ] - `p1` - API Server delegates to Transfer Orchestrator ListTransfers with filter criteria - `inst-delegate-list`
4. [ ] - `p1` - **RETURN** 200 with transfer list including id, status, source, destination, created_at, and pagination metadata - `inst-return-list`

### Get Transfer Detail

- [ ] `p1` - **ID**: `cpt-katapult-flow-api-cli-get-transfer`
...
**Steps**:
1. [ ] - `p1` - Operator sends GET /api/v1alpha1/transfers/{id} or runs `katapult transfer get <id>` - `inst-query-detail`
2. [ ] - `p1` - API Server authenticates and authorizes request - `inst-auth-detail`
3. [ ] - `p1` - API Server delegates to Transfer Orchestrator GetTransfer - `inst-delegate-get`
4. [ ] - `p1` - **IF** transfer not found **RETURN** 404 with descriptive message - `inst-not-found`
5. [ ] - `p1` - **RETURN** 200 with transfer detail including progress (bytes_transferred, total_bytes, speed, eta, chunks_completed, chunks_total), strategy, events - `inst-return-detail`

### Cancel Transfer via API

- [ ] `p1` - **ID**: `cpt-katapult-flow-api-cli-cancel-transfer`
...
**Steps**:
1. [ ] - `p1` - Operator sends DELETE /api/v1alpha1/transfers/{id} or runs `katapult transfer cancel <id>` - `inst-cancel-request`
2. [ ] - `p1` - API Server authenticates and authorizes request (require operator role) - `inst-auth-cancel`
3. [ ] - `p1` - API Server delegates to Transfer Orchestrator CancelTransfer - `inst-delegate-cancel`
4. [ ] - `p1` - **IF** transfer not found **RETURN** 404 - `inst-cancel-not-found`
5. [ ] - `p1` - **IF** transfer already in terminal state (Completed, Failed, Cancelled) **RETURN** 409 Conflict - `inst-cancel-conflict`
6. [ ] - `p1` - **RETURN** 200 with updated transfer status=Cancelled - `inst-return-cancelled`

### List Agents and PVCs

- [ ] `p1` - **ID**: `cpt-katapult-flow-api-cli-list-agents`
...
**Steps**:
1. [ ] - `p1` - Operator sends GET /api/v1alpha1/agents or runs `katapult agent list` - `inst-query-agents`
2. [ ] - `p1` - API Server authenticates and authorizes request (viewer or operator role) - `inst-auth-agents`
3. [ ] - `p1` - API Server delegates to Agent Registry via `cpt-katapult-component-agent-registry` ListAgents - `inst-delegate-agents`
4. [ ] - `p1` - **RETURN** 200 with agent list including id, cluster, node, status, last_heartbeat, pvc_count - `inst-return-agents`
5. [ ] - `p1` - Operator optionally sends GET /api/v1alpha1/agents/{id}/pvcs or runs `katapult agent get <id>` for PVC detail - `inst-query-pvcs`
6. [ ] - `p1` - **RETURN** 200 with PVC list including name, namespace, size, storage_class, node_affinity, bound_pv - `inst-return-pvcs`

### Create Transfer via CRD

- [ ] `p1` - **ID**: `cpt-katapult-flow-api-cli-create-transfer-crd`
...
**Steps**:
1. [ ] - `p1` - Operator applies VolumeTransfer CR (katapult.io/v1alpha1) via kubectl - `inst-apply-crd`
2. [ ] - `p1` - CRD Controller via `cpt-katapult-component-crd-controller` receives reconciliation event - `inst-reconcile-event`
3. [ ] - `p1` - CRD Controller extracts spec (source, destination, strategy, allowOverwrite, retry) - `inst-extract-spec`
4. [ ] - `p1` - CRD Controller calls Transfer Orchestrator CreateTransfer using `cpt-katapult-algo-api-cli-reconcile-crd` - `inst-crd-create`
5. [ ] - `p1` - **IF** Orchestrator returns error, CRD Controller sets status condition type=Ready, status=False, reason=CreateFailed - `inst-crd-error`
6. [ ] - `p1` - CRD Controller updates status subresource with phase, progress, and conditions from transfer state - `inst-update-status`
7. [ ] - `p1` - CRD Controller re-enqueues reconciliation to poll transfer state until terminal - `inst-requeue`

## 3. Processes / Business Logic (CDSL)

### Validate Transfer Request

- [ ] `p1` - **ID**: `cpt-katapult-algo-api-cli-validate-transfer-request`
...
**Steps**:
1. [ ] - `p1` - **IF** source cluster or source PVC reference is empty **RETURN** error "source cluster and PVC are required" - `inst-check-source`
2. [ ] - `p1` - **IF** destination cluster or destination PVC reference is empty **RETURN** error "destination cluster and PVC are required" - `inst-check-dest`
3. [ ] - `p1` - **IF** source and destination refer to the same cluster and PVC **RETURN** error "source and destination must differ" - `inst-check-same`
4. [ ] - `p1` - **IF** strategy is provided AND strategy not in [stream, s3, direct] **RETURN** error "invalid strategy" - `inst-check-strategy`
5. [ ] - `p1` - **IF** retry.maxAttempts is provided AND retry.maxAttempts < 1 **RETURN** error "maxAttempts must be >= 1" - `inst-check-retry`
6. [ ] - `p1` - Query Agent Registry for source agent that owns the source PVC - `inst-lookup-source-agent`
7. [ ] - `p1` - **IF** source agent not found or unhealthy **RETURN** error "source agent unavailable" - `inst-check-source-agent`
8. [ ] - `p1` - Query Agent Registry for destination agent that owns the destination PVC - `inst-lookup-dest-agent`
9. [ ] - `p1` - **IF** destination agent not found or unhealthy **RETURN** error "destination agent unavailable" - `inst-check-dest-agent`
10. [ ] - `p1` - **IF** allowOverwrite is false, query destination agent to verify destination PVC is empty - `inst-check-dest-empty`
11. [ ] - `p1` - **IF** destination PVC is not empty AND allowOverwrite is false **RETURN** error "destination PVC is not empty; set allowOverwrite to proceed" - `inst-dest-not-empty`
12. [ ] - `p1` - **RETURN** valid - `inst-return-valid`

### Resolve CLI Command

- [ ] `p2` - **ID**: `cpt-katapult-algo-api-cli-resolve-cli-command`
...
**Steps**:
1. [ ] - `p1` - Parse command and subcommand from argv (transfer create|list|get|cancel, agent list|get) - `inst-parse-command`
2. [ ] - `p1` - **IF** command is unknown **RETURN** usage help with available commands - `inst-unknown-command`
3. [ ] - `p1` - Parse flags and positional arguments per command schema - `inst-parse-flags`
4. [ ] - `p1` - **IF** required flags missing **RETURN** error with missing flag names and usage hint - `inst-missing-flags`
5. [ ] - `p1` - Resolve API server address from --server flag, KATAPULT_SERVER env var, or kubeconfig current context - `inst-resolve-address`
6. [ ] - `p1` - Resolve authentication token from --token flag, KATAPULT_TOKEN env var, or kubeconfig auth info - `inst-resolve-auth`
7. [ ] - `p1` - Construct HTTP request (method, path, headers, body) for the resolved command - `inst-construct-http`
8. [ ] - `p1` - **RETURN** constructed request - `inst-return-request`

### Reconcile VolumeTransfer CRD

- [ ] `p1` - **ID**: `cpt-katapult-algo-api-cli-reconcile-crd`
...
**Steps**:
1. [ ] - `p1` - Read VolumeTransfer CR spec (source, destination, strategy, allowOverwrite, retry) - `inst-read-spec`
2. [ ] - `p1` - **IF** status.transferID is empty (new CR), call Transfer Orchestrator CreateTransfer with spec fields - `inst-create-transfer`
3. [ ] - `p1` - **IF** CreateTransfer fails, set status condition type=Ready, status=False, reason=CreateFailed, message=error detail - `inst-create-failed`
4. [ ] - `p1` - **IF** CreateTransfer succeeds, store transfer ID in status.transferID - `inst-store-id`
5. [ ] - `p1` - **IF** status.transferID is present (existing CR), query Transfer Orchestrator GetTransfer for current state - `inst-poll-state`
6. [ ] - `p1` - Update status.phase from transfer state (Pending, Validating, Transferring, Completed, Failed, Cancelled) - `inst-update-phase`
7. [ ] - `p1` - Update status.progress from transfer progress (bytes_transferred, total_bytes, speed, chunks) - `inst-update-progress`
8. [ ] - `p1` - Update status.conditions with appropriate Kubernetes condition entries - `inst-update-conditions`
9. [ ] - `p1` - **IF** transfer is in terminal state (Completed, Failed, Cancelled) **RETURN** do not requeue - `inst-terminal`
10. [ ] - `p1` - **RETURN** requeue after interval (default 5s) - `inst-requeue`
...
- [ ] `p1` - **ID**: `cpt-katapult-dod-api-cli-rest-transfer-endpoints`
...
- [ ] `p1` - **ID**: `cpt-katapult-dod-api-cli-rest-agent-endpoints`
...
- [ ] `p1` - **ID**: `cpt-katapult-dod-api-cli-cli-tool`
...
- [ ] `p1` - **ID**: `cpt-katapult-dod-api-cli-crd-controller`
...
- [ ] `p1` - **ID**: `cpt-katapult-dod-api-cli-input-validation`
...
- [ ] `p1` - **ID**: `cpt-katapult-dod-api-cli-auth`
